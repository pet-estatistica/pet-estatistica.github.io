...
* **Aritmética modular** - As operações resto (%%) e divisão inteira (%/%) funcionam da mesma maneira que as operações aritméticas;
* **Operações de desvio (lag e lead)** - Essas operações alteram a ordem da posição dos elementos dos vetores: a função lead() faz o primeiro valor receber o segundo, o segundo receber o terceiro, e assim por diante, até o último valor, que será eliminado (retorna NA). De forma similar, lag() irá atribuir o penúltimo valor para o último, o ante-penúltimo para o penúltimo, e assim por diante, até o primeiro valor, que será eliminado. Veja o exemplo:
...
* **Valores Acumulados e Agregados Sequênciais** - As funções cumsum, cumprod, cummin e cummax proporcionam uma soma e produto cumulativas, mínimo e máximo sequênciais, e a função cummean fornece a média móvel dos dados. Veja o exemplo:
